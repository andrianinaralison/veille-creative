  ...
  return (
    <div className="bg-canvas min-h-screen animate-fade-in">
      ...
      <div className="px-8 pt-8 pb-6 border-b border-surface-border">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <h1 className="font-editorial text-4xl text-ink">Découvertes</h1>
            </div>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 border border-surface-border text-sm text-ink-muted hover:text-ink hover:border-ink/20 transition-all ${refreshing ? 'opacity-50' : ''}`}
            disabled={refreshing}
          >
        ...
        {(liked > 0 || disliked > 0) && (
          <div className="flex items-center gap-4 mt-5 p-3 bg-surface-raised border border-surface-border">
            <div className="font-mono text-xs text-ink-muted">Vos retours :</div>
            {liked > 0 && (
      ...
      <div className="px-8 py-8">
        <div className="max-w-2xl mx-auto space-y-8">
          {surprises.map(surprise => (
            <div key={surprise.id} className="border border-surface-border bg-surface overflow-hidden animate-slide-up">
              <div className="relative aspect-[16/7]">
                <img src={surprise.thumbnail} alt={surprise.title} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />

                <div className="absolute top-4 left-4 flex items-center gap-2">
                  <span className="font-mono text-[10px] font-semibold px-2.5 py-1 bg-ink text-canvas">
                    {surprise.origin}
                  </span>
                  <span className="font-mono text-[10px] text-white/60">{surprise.platform}</span>
                </div>

                <button
                  onClick={() => handleSave(surprise.id)}
                  className={`absolute top-4 right-4 w-8 h-8 flex items-center justify-center transition-all backdrop-blur-md ${
                    surprise.saved ? 'bg-ink text-canvas' : 'bg-black/40 text-white hover:bg-ink hover:text-canvas'
                  }`}
                >
              ...
              <div className="p-5">
                ...
                <div className="flex gap-1.5 mb-5 flex-wrap">
                  {surprise.tags.map(tag => (
                    <span key={tag} className="text-[11px] px-2 py-0.5 bg-surface-raised text-ink-muted border border-surface-border">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-surface-border">
                  ...
                  <div className="flex gap-2 ml-auto">
                    ...
                    >
                      <ThumbsDown size={13} /> Pas pour moi
                    </button>
                  </div>
                </div>

                {surprise.reaction && (
                  <div className="mt-3 p-2.5 bg-surface-raised border border-surface-border text-[11px] text-ink-muted">
                    {surprise.reaction === 'like'
                      ? '✓ Noté — vos prochaines suggestions intégreront des directions similaires.'
        ...
        {surprises.every(s => s.reaction) && (
          <div className="mt-8 max-w-2xl mx-auto text-center p-8 border border-dashed border-ink/15">
            <Sparkles size={24} className="mx-auto text-ink-muted mb-3" />
            <div className="font-editorial text-xl text-ink mb-2">Vous avez évalué toutes les découvertes</div>
            <p className="text-xs text-ink-muted mb-4">Vos retours nous permettent d'affiner les prochaines suggestions.</p>
            <button
              onClick={handleRefresh}
              className="flex items-center gap-2 px-4 py-2 bg-ink/15 text-ink text-sm font-medium mx-auto hover:bg-ink/20 transition-all"
            >
              <RefreshCw size={14} /> Voir de nouvelles découvertes
            </button>
          </div>
        )}
      </div>
