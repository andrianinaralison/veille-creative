...
import { Clock, Bookmark, BookmarkCheck, Sparkles, ThumbsUp, ThumbsDown } from 'lucide-react'
import { mockDigest, mockSurprises } from '../data/mockData'

function DigestItem({ item }) {
  const [saved, setSaved] = useState(item.saved)
  return (
    <div className="group grid grid-cols-[1fr_2fr] gap-6 p-5 border border-surface-border hover:border-ink/15 bg-surface transition-all">
      ...
      <div className="flex flex-col justify-between py-1">
          ...
          <div className="flex items-start justify-between gap-3 mb-2">
            ...
            <button
              ...
              className={`flex-shrink-0 w-8 h-8 flex items-center justify-center transition-all ${
                saved ? 'bg-ink text-canvas' : 'bg-surface-raised text-ink-muted hover:bg-ink/10 hover:text-ink'
              }`}
            >
        ...
        <div className="flex items-center gap-3 mt-4">
          <span className="font-mono text-[10px] text-ink-faint uppercase tracking-wider">{item.platform}</span>
          <span className="text-ink-faint">·</span>
          <div className="flex gap-1.5">
            {item.tags.slice(0, 3).map(tag => (
              <span key={tag} className="text-[11px] px-2 py-0.5 bg-surface-raised text-ink-muted border border-surface-border">{tag}</span>
            ))}
          </div>
          {saved && <span className="ml-auto text-[11px] text-ink font-medium">✓ Sauvegardé</span>}
        </div>
      </div>
    </div>
  )
    ...
    <div className="bg-canvas min-h-screen animate-fade-in">
      ...
      <div className="px-10 pt-10 pb-8 border-b border-surface-border">
        <div className="flex items-start justify-between">
          <div>
            <p className="font-mono text-[11px] text-ink-muted uppercase tracking-widest font-semibold mb-3">· {mockDigest.week}</p>
          ...
          {/* Tabs */}
          <div className="flex gap-1 p-1 bg-surface-raised border border-surface-border">
            <button
              onClick={() => setTab('digest')}
              className={`px-4 py-2 text-sm font-medium transition-all ${tab === 'digest' ? 'bg-ink text-canvas' : 'text-ink-muted hover:text-ink'}`}
            >
              Digest
            </button>
            <button
              onClick={() => setTab('surprises')}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-medium transition-all ${tab === 'surprises' ? 'bg-ink text-canvas' : 'text-ink-muted hover:text-ink'}`}
            >
              ...
              return (
                <div key={section.id}>
                  <div className="flex items-baseline gap-4 mb-6">
                    <span className="font-editorial text-5xl text-ink-faint">{meta.num}</span>
        ...
        {tab === 'surprises' && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-3 gap-5">
              {mockSurprises.map(s => (
                <div key={s.id} className="overflow-hidden border border-surface-border bg-surface">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium border transition-all ${
                          reactions[s.id] === 'like'
                      ...
                      <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium border transition-all ${
                          reactions[s.id] === 'dislike'
                            ? 'border-red-500/50 bg-red-500/10 text-red-400'
                            : 'border-surface-border text-ink-muted hover:border-red-500/30 hover:text-red-400'
                        }`}
                      >
