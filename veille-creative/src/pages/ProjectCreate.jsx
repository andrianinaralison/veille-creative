        ...
        <div className="w-full max-w-lg">
          ...
          <div className="flex items-center gap-3 mb-10">
            {steps.map((s, i) => (
              <div key={s} className="flex items-center gap-3">
                <div className={`flex items-center justify-center w-7 h-7 text-xs font-semibold transition-all ${
                  i < step
                    ? 'bg-ink text-canvas'
                    : i === step
            ...
            <div className="space-y-5 animate-slide-up">
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-surface border border-surface-border text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:border-ink/30 transition-colors"
                  />
                </div>
              ))}
              <div>
                ...
                <textarea
                  placeholder="Objectif, ton, diffusion, contraintes..."
                  value={form.brief}
                  onChange={e => setForm(p => ({ ...p, brief: e.target.value }))}
                  rows={5}
                  className="w-full px-4 py-3 bg-surface border border-surface-border text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:border-ink/30 transition-colors resize-none"
                />
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-surface border border-surface-border text-sm text-ink-muted focus:outline-none focus:border-ink/30 transition-colors"
                />
              </div>
            </div>
          )}
                ...
                {moodPresets.map(preset => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 border text-sm font-medium transition-all ${
                      form.moodPresets.includes(preset.id)
            ...
            <div className="animate-slide-up">
              ...
              <div className="space-y-4">
                {[['Titre', form.title], ['Client', form.client || '—'], ['Brief', form.brief]].map(([label, val]) => (
                  <div key={label} className="p-4 bg-surface border border-surface-border">
                    <div className="font-mono text-[11px] text-ink-muted mb-1 uppercase tracking-wider">{label}</div>
                    <div className="text-sm text-ink">{val}</div>
                  </div>
                ))}
                {form.moodPresets.length > 0 && (
                  <div className="p-4 bg-surface border border-surface-border">
                    <div className="font-mono text-[11px] text-ink-muted mb-2 uppercase tracking-wider">Ambiances</div>
                    <div className="flex gap-2 flex-wrap">
                      {form.moodPresets.map(id => {
                        const p = moodPresets.find(x => x.id === id)
                        return (
                          <span key={id} className="text-xs px-3 py-1 border border-ink/25 bg-ink/10 text-ink">
                            {p?.label}
                          </span>
                        )
                      })}
                ...
                <div className="p-4 border border-ink/15 bg-ink/5 flex items-start gap-3">
                  <Zap size={14} className="text-ink mt-0.5 flex-shrink-0" />
                  <div>
                    <div className="text-xs font-semibold text-ink mb-0.5">Exploration IA prête</div>
          ...
          <div className="flex items-center justify-between mt-10">
            <button
              ...
              className="px-4 py-2.5 border border-surface-border text-sm text-ink-muted hover:text-ink hover:border-ink/20 transition-all"
            >
            ...
            {step < steps.length - 1 ? (
              <button
                ...
                disabled={!canNext()}
                className="flex items-center gap-2 px-6 py-2.5 bg-ink hover:opacity-90 disabled:opacity-30 text-canvas text-sm font-semibold transition-opacity"
              >
                Continuer <ChevronRight size={15} />
              </button>
            ) : (
              <button
                onClick={() => navigate('/projects/proj-new')}
                className="flex items-center gap-2 px-6 py-2.5 bg-ink hover:opacity-90 text-canvas text-sm font-semibold transition-opacity"
              >
